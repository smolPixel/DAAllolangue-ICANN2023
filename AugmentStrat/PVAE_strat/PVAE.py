...
from AugmentStrat.PVAE_strat.utils import to_var,expierment_name
from AugmentStrat.PVAE_strat.model import SentenceVAE
...
class PVAE_meta():
    ...
    def init_model_dataset(self):
        ...
        try:
            model = torch.load(f"/data/rali6/Tmp/piedboef/Models/DACon/PVAE_{self.language}_{self.argdict['strat_pretraining']}_{self.argdict['tokenizer']}.pt")
            self.pretraining = False
        except:
            self.model = SentenceVAE(**params).cuda()
            self.optimizer = torch.optim.Adam(self.model.parameters(), lr=0.00005)
            self.pretrain_dataset=initialize_dataset_pretraining(self.argdict, self.language)
            self.pretrain_model()
            torch.save(self.model, f"/data/rali6/Tmp/piedboef/Models/DACon/PVAE_{self.language}_{self.argdict['strat_pretraining']}_{self.argdict['tokenizer']}.pt")
            self.pretraining = False
            model=self.model
    ...
    def loss_fn(self, logp, target, mean, logv, anneal_function, step, k, pretraining=False):
    ...
    def pretrain_model(self):
        for epoch in range(self.argdict['nb_epoch_algo_pretraining']):
            ...
            for iteration, batch in enumerate(data_loader):
                ...
                # Forward pass
                input = batch['input'][:, :-1]
                # logp, mean, logv, z = self.model(input, pretraining=True)
                logp, _,_,_  = self.model(input, pretraining=True)
                # print(batch['input'].shape)
                target = batch['input'][:, 1:]
                # print(target.shape)
                # loss calculation
            ...
            torch.save(self.model, f"/data/rali6/Tmp/piedboef/Models/DACon/PVAE_{self.language}_{self.argdict['strat_pretraining']}_{self.argdict['tokenizer']}_Ep{epoch}.pt")
