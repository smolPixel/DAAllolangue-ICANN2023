...
class Bert_Classifier():
    ...
    def run_epoch(self, train, dev, test, return_grad=False):
        ...
        if return_grad:
            ...
            return accuracy_score(Y_train, pred_train), accuracy_score(Y_dev, pred_dev),accuracy_score(Y_test, pred_test), 0, 0, 0, 0
    ...
    def train_model(self, train, dev, test, type='dataLoader', generator=None, return_grad=False):
        ...
        for j in range(self.argdict['nb_epoch_classifier']):
            acc_train, acc_dev, acc_test, grad_base, grad_aug, acc_base, acc_aug=self.run_epoch(train, dev, test, return_grad=return_grad)
            ...
            print(f"Epoch {j}, accuracy on dev {(acc_train, acc_dev)} grad base {grad_base} grad aug {grad_aug}"
    ...
    def train_batches(self, train, dev):
        ...
        for i, batch in enumerate(data_loader):
            ...
            for j in range(self.argdict['nb_epoch_classifier']):
                ...
                self.optimizer.step()
                acc_train=accuracy_score(batch['label'].cpu(), results.cpu())
